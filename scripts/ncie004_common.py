"""Shared rendering engine for NCIE-004 Technology Stack & Engineering Blueprint.

Reuses the NCIE-001/002/003 brand system. Deliberately leaner than the
NCIE-003 engine: per-chapter content is compact (upstream references +
one technology-decision table + short engineering notes + concise Human
Review Focus), per the token-conservation directive in the NCIE-004
handover instructions. Consolidated governance (Technology Decision
Matrix, Technology Registry, Traceability, Acceptance) lives in Ch.33/35/36
only, not repeated per chapter.
"""
...
DOC_ID = "NCIE-004"
DOC_TITLE = "Technology Stack & Engineering Blueprint"
DOC_VERSION = "Version 1.1"
DOC_STATUS = "In development — for human review"
TOTAL_CHAPTERS = 36

# Technology-selection status classification (handover §5)
TECH_STATUS = {
    "MANDATE": "APPROVED UPSTREAM MANDATE",
    "PROPOSED": "PROPOSED DESIGN DEFAULT",
    "INSTITUTIONAL": "INSTITUTIONAL CONFIRMATION REQUIRED",
    "INFRASTRUCTURE": "INFRASTRUCTURE DISCOVERY REQUIRED",
...
# ---------------------------------------------------------------------------
# Styles / page / header-footer (identical pattern to NCIE-002/003)
# ---------------------------------------------------------------------------
...
def add_header_footer(section):
    ...
    run = p.add_run(f"{DOC_ID}  |  TECHNOLOGY STACK & ENGINEERING BLUEPRINT  |  {DOC_VERSION.upper()}")
...
def add_cover(document):
    ...
    meta_lines = [
        "Selects and governs technology families and engineering patterns implementing NCIE-002/003",
        "Named vendors/products remain proposed defaults until institutionally approved",
        "Traces to NCIE-001 (requirements), NCIE-002 v0.4 (architecture) and NCIE-003 v0.3 (canonical data)",
...
def add_data_table(document, headers, rows, caption=None, font_size=8.0, header_size=7.6, status_col=None):
    """status_col: index of a column whose values are TECH_STATUS keys/labels, colour-coded."""
    _table_counter["n"] += 1
    if caption:
    ...
    for r_index, row_values in enumerate(rows, start=1):
        ...
        for c_index, (cell, value) in enumerate(zip(row.cells, row_values)):
            ...
            if status_col is not None and c_index == status_col:
                for key, label in TECH_STATUS.items():
                    if text.strip().upper() in (key, label):
                        text = label
                        color = TECH_STATUS_COLOR[key]
                        bold = True
                        break
            elif text.strip().upper() == "SOURCE DISCOVERY REQUIRED":
                color = FOREST
                bold = True
...
# ---------------------------------------------------------------------------
# Status / proposed / upstream / trace blocks
# ---------------------------------------------------------------------------
...
def add_upstream(document, items):
    """items: list of (doc_id, ref, consequence) e.g. ('NCIE-002','Ch.7','...')"""
    ...
    parts = []
    for doc_id, ref, consequence in items:
        parts.append(f"{doc_id} {ref} — {consequence}")
