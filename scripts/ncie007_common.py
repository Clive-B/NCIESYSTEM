"""Shared rendering engine for NCIE-007 AI, Agent & Model Orchestration
Specification. Reuses the NCIE-001-006 brand system. Token-conserving:
chapters reference upstream rather than reproduce it. Adds a 7-category
Human Review classification per the NCIE-007 handover instructions (item
1411): RESOLVED, PROPOSED, INSTITUTIONAL, SECURITY, LEGAL, INFRASTRUCTURE,
WORKFLOW. Adds a "vpf" block for VPF-disposition callouts (mirrors NCIE-006's
"voice" block) and an "agentdef" block for Agent Definition Package fields.
"""
...
DOC_ID = "NCIE-007"
DOC_TITLE = "AI, Agent & Model Orchestration Specification"
DOC_VERSION = "Version 1.1"
DOC_STATUS = "In development — for human review"
TOTAL_CHAPTERS = 36

# Human Review Focus classification (handover item 1411)
REVIEW_STATUS = {
    "RESOLVED": "RESOLVED BY UPSTREAM BASELINE",
    "PROPOSED": "PROPOSED DESIGN DEFAULT",
    "INSTITUTIONAL": "INSTITUTIONAL CONFIRMATION REQUIRED",
    "SECURITY": "SECURITY/SOVEREIGNTY CONFIRMATION REQUIRED",
    "LEGAL": "LEGAL/POLICY CONFIRMATION REQUIRED",
    "INFRASTRUCTURE": "ENGINEERING/INFRASTRUCTURE DISCOVERY REQUIRED",
    "WORKFLOW": "SOURCE/WORKFLOW DISCOVERY REQUIRED",
}
REVIEW_STATUS_COLOR = {
    "RESOLVED": FOREST,
    "PROPOSED": GOLD,
    "INSTITUTIONAL": DEEP_GREEN,
    "SECURITY": MID_GREY,
    "LEGAL": CHARCOAL,
...
def configure_extra_styles(document):
    ...
    proposed.paragraph_format.space_before = Pt(4)
    proposed.paragraph_format.space_after = Pt(6)
    ...
    trace.paragraph_format.space_after = Pt(6)

    upstream = ensure_paragraph_style(document, "NCIE Upstream Block")
    upstream.font.name = "Aptos"
    upstream._element.rPr.rFonts.set(qn("w:eastAsia"), "Aptos")
    upstream.font.size = Pt(8.8)
    upstream.font.color.rgb = RGBColor.from_string(FOREST)
    upstream.paragraph_format.space_before = Pt(2)
    upstream.paragraph_format.space_after = Pt(3)

    vpf = ensure_paragraph_style(document, "NCIE VPF Block")
    vpf.font.name = "Aptos"
    vpf._element.rPr.rFonts.set(qn("w:eastAsia"), "Aptos")
    vpf.font.size = Pt(8.8)
    vpf.font.color.rgb = RGBColor.from_string(CHARCOAL)
    vpf.paragraph_format.left_indent = Cm(0.35)
    vpf.paragraph_format.space_before = Pt(3)
    vpf.paragraph_format.space_after = Pt(6)
...
def add_header_footer(section):
    ...
    run = p.add_run(f"{DOC_ID}  |  AI, AGENT & MODEL ORCHESTRATION SPECIFICATION  |  {DOC_VERSION.upper()}")
...
def add_cover(document):
    ...
    meta_lines = [
        "Specifies the orchestration layer beneath ARGUS: model routing, agents, Dynamic Agent Synthesis, tools, VPF output validation, delegation and failure containment",
        "Model-independent: ARGUS is not a model, agent output is not institutional authority, and VPF validation is not human approval",
        "Traces to NCIE-002 v0.4 (architecture), NCIE-003 v0.3 (canonical data), NCIE-004 v1.1 (engineering), NCIE-005 v1.2 (ARGUS behavior) and NCIE-006 v1.1 (Memory & Context)",
...
def add_data_table(document, headers, rows, caption=None, font_size=8.0, header_size=7.6, status_col=None):
    _table_counter["n"] += 1
    if caption:
    ...
    for r_index, row_values in enumerate(rows, start=1):
        ...
        for c_index, (cell, value) in enumerate(zip(row.cells, row_values)):
            set_cell_shading(cell, fill)
            text = str(value)
            bold = False
            color = CHARCOAL
            if status_col is not None and c_index == status_col:
                for key, label in REVIEW_STATUS.items():
                    if text.strip().upper() in (key, label):
                        text = label
                        color = REVIEW_STATUS_COLOR[key]
                        bold = True
                        break
            set_cell_text(cell, text, size=font_size, color=color, bold=bold)
    document.add_paragraph()
    return table
...
# ---------------------------------------------------------------------------
# Status / proposed / upstream / vpf / trace blocks
# ---------------------------------------------------------------------------
...
def add_vpf(document, text):
    p = document.add_paragraph(style="NCIE VPF Block")
    set_paragraph_shading(p, "F5EFE0")
    set_paragraph_left_border(p, color=GOLD, size="16")
    label = p.add_run("VPF ARCHITECTURE.  ")
    style_run(label, size=8.8, color=DEEP_GREEN, bold=True)
    run = p.add_run(text)
    style_run(run, size=8.8, color=CHARCOAL, bold=False)
    return p


def add_review_item(document, category, text):
    label_text = REVIEW_STATUS[category]
    color = REVIEW_STATUS_COLOR[category]
    p = document.add_paragraph(style="List Bullet")
    label = p.add_run(f"[{label_text}]  ")
    style_run(label, size=9.2, color=color, bold=True)
    run = p.add_run(text)
    style_run(run, size=10, color=CHARCOAL, bold=False)
    return p
...
def add_review_focus(document, items):
    ...
    for item in items:
        if isinstance(item, tuple):
            add_review_item(document, item[0], item[1])
        else:
...
def render_blocks(document, blocks):
    for block in blocks:
        ...
        if kind == "h2":
            ...
            _add_upstream(document, block[1])
        elif kind == "vpf":
            add_vpf(document, block[1])
        elif kind == "proposed":
            ...
            add_data_table(document, headers, rows, caption, status_col=status_col)
        elif kind == "pagebreak":
            document.add_page_break()
        else:
...
def _add_upstream(document, items):
    p = document.add_paragraph(style="NCIE Upstream Block")
    label = p.add_run("Upstream: ")
    style_run(label, size=8.8, color=DEEP_GREEN, bold=True)
    parts = [f"{doc_id} {ref} — {consequence}" for doc_id, ref, consequence in items]
    run = p.add_run("  |  ".join(parts))
    style_run(run, size=8.8, color=FOREST, bold=False)
    return p
