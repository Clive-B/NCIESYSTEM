"""Shared rendering engine for NCIE-006 Memory & Context Architecture
Specification. Reuses the NCIE-001-005 brand system. Token-conserving:
chapters reference upstream rather than reproduce it. Adds a 6-category
Human Review classification (the 5 from NCIE-003/005 plus SECURITY/
SOVEREIGNTY CONFIRMATION REQUIRED) per the NCIE-006 handover instructions.
"""
...
DOC_ID = "NCIE-006"
DOC_TITLE = "Memory & Context Architecture Specification"
DOC_VERSION = "Version 1.1"
DOC_STATUS = "In development — for human review"
TOTAL_CHAPTERS = 36

# Human Review Focus classification (handover item 48)
REVIEW_STATUS = {
...
def configure_extra_styles(document):
    ...
    voice = ensure_paragraph_style(document, "NCIE Voice Block")
    voice.font.name = "Aptos"
    voice._element.rPr.rFonts.set(qn("w:eastAsia"), "Aptos")
    voice.font.size = Pt(8.8)
    voice.font.color.rgb = RGBColor.from_string(CHARCOAL)
    voice.paragraph_format.left_indent = Cm(0.35)
    voice.paragraph_format.space_before = Pt(3)
    voice.paragraph_format.space_after = Pt(6)
...
def add_header_footer(section):
    ...
    run = p.add_run(f"{DOC_ID}  |  MEMORY & CONTEXT ARCHITECTURE SPECIFICATION  |  {DOC_VERSION.upper()}")
...
def add_cover(document):
    ...
    meta_lines = [
        "Consolidates persistent Memory and Context architecture: classes, temporal integrity, provenance, authorization and Institutional Knowledge lifecycle",
        "Model-independent: Memory survives ARGUS model/provider replacement and does not depend on any conversation-history feature",
        "Traces to NCIE-002 v0.4 (architecture), NCIE-003 v0.3 (canonical data), NCIE-004 v1.1 (engineering) and NCIE-005 v1.2 (ARGUS behavior)",
...
# ---------------------------------------------------------------------------
# Status / proposed / upstream / voice / trace blocks
# ---------------------------------------------------------------------------
...
def add_voice(document, text):
    p = document.add_paragraph(style="NCIE Voice Block")
    set_paragraph_shading(p, "F5EFE0")
    set_paragraph_left_border(p, color=GOLD, size="16")
    label = p.add_run("VOICE-DERIVED MEMORY.  ")
