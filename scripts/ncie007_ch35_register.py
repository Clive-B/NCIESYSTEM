"""NCIE-007 Chapter 35 master Human Review Register — extracted directly
...
NCIE-004 Ch.35, NCIE-005 Ch.34 and NCIE-006 Ch.35; mandatory per handover
items 1311/1369)."""

import importlib

from ncie007_common import REVIEW_STATUS
...
BLOCKING_STATUS = {"INSTITUTIONAL", "SECURITY", "LEGAL", "INFRASTRUCTURE", "WORKFLOW"}
        ...
        for chnum, blocks in sorted(module.BLOCKS.items()):
            for block in blocks:
                ...
                for entry in block[1]:
                    ...
                    items.append({
                        "id": f"HR7-{chnum}-{seq_by_chapter[chnum]}",
                        "chapter": chnum,
                        "category": category,
                        "issue": text,
                    })
...
def summary_counts():
    ...
    return {
        ...
        "infrastructure": cat_counts.get("INFRASTRUCTURE", 0),
