"""Shared rendering engine for NCIE-005 ARGUS Intelligence Assistant Specification.

Reuses the NCIE-001/002/003/004 brand system. Token-conserving per the
handover instructions: chapters reference upstream rather than reproduce
it, and a dedicated "voice" block renders the skeleton's per-chapter Voice
Requirement clauses distinctly from ordinary upstream references.
"""
...
DOC_ID = "NCIE-005"
DOC_TITLE = "ARGUS Intelligence Assistant Specification"
DOC_VERSION = "Version 1.2"
DOC_STATUS = "In development — for human review"
TOTAL_CHAPTERS = 35

# Human Review Focus classification (handover §32)
REVIEW_STATUS = {
    "PROPOSED": "PROPOSED DESIGN DEFAULT",
    "INSTITUTIONAL": "INSTITUTIONAL CONFIRMATION REQUIRED",
    "WORKFLOW": "WORKFLOW/SOURCE DISCOVERY REQUIRED",
...
def add_header_footer(section):
    ...
    run = p.add_run(f"{DOC_ID}  |  ARGUS INTELLIGENCE ASSISTANT SPECIFICATION  |  {DOC_VERSION.upper()}")
...
def add_cover(document):
    ...
    meta_lines = [
        "Defines ARGUS behavior, interaction, analytical participation and evidence-bounded response under Human-Primary authority",
        "Voice Interaction Update: spoken/verbal interaction is a mandatory interaction modality, never an authority modality",
        "Traces to NCIE-001 (requirements), NCIE-002 v0.4 (architecture), NCIE-003 v0.3 (canonical data) and NCIE-004 v1.1 (engineering baseline)",
...
def add_upstream(document, items):
...
def add_voice(document, text):
    ...
    label = p.add_run("VOICE REQUIREMENT.  ")
    style_run(label, size=8.8, color=DEEP_GREEN, bold=True)
    run = p.add_run(text)
